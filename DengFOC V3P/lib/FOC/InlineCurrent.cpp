...
void CurrSense::configureADCInline(const int pinA, const int pinB, const int pinC)
{
	...
	//adc_set_clk_div(80);				// 分频80，采样时钟1MHz，更快（噪声略增）
	if (pinA == 39 && pinB == 36)
	{
		adc1_config_channel_atten(ADC1_CHANNEL_3, ADC_ATTEN_11db); // 39
		adc1_config_channel_atten(ADC1_CHANNEL_0, ADC_ATTEN_11db); // 36
		adc1_get_raw(ADC1_CHANNEL_3);
		adc1_get_raw(ADC1_CHANNEL_0);
	}
	else
	{
		adc1_config_channel_atten(ADC1_CHANNEL_7, ADC_ATTEN_11db); // 35
		adc1_config_channel_atten(ADC1_CHANNEL_6, ADC_ATTEN_11db); // 34
