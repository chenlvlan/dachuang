...
static void _TransmitProtocolData(UartProtocolData_t *pData) {
	uint16_t tmp_crc = CalcCRC16_Modbus(comm_uart_tx_buffer,
			(pData->data_len + 5));
	...
	if (HAL_OK
			!= UART_Transmit_DMA(&huart3, comm_uart_tx_buffer,
					pData->data_len + 7)) {
		UART_EN_RS485_RX(&huart3);
	}
}
...
/* 写运动控制参数 */
void CommRs485_WriteMotionCtrlPara(uint8_t dev_addr, MotionCtrlPara_t pid,
		bool is_save) {
	UartProtocolData_t *pData = (UartProtocolData_t*) comm_uart_tx_buffer;
	uint32_t tmp_val;
	...
	pData->cmd =
			(is_save == true) ?
					eWriteAndSaveMotionCtrlPara : eWriteMotionCtrlPara;
	pData->data_len = 0x18;
...
void CommRs485_VelocityCtrl(uint8_t dev_addr, float target_vel,
		float target_acc) {
	UartProtocolData_t *pData = (UartProtocolData_t*) comm_uart_tx_buffer;
...
void UART_IDLE_IT_Callback(UART_HandleTypeDef *huart) {
	...
	if (UART_GET_FLAG(huart, UART_FLAG_IDLE) != RESET) {
		...
		if (size != 0) {
			...
			if ((size >= 7)
					&& (CalcCRC16_Modbus(comm_uart_rx_buffer, size) == 0)) {
				UartProtocolData_t *pData =
						(UartProtocolData_t*) comm_uart_rx_buffer;
				if (pData->head == UART_RX_PROTOCOL_HEADER_V3) {
					switch (pData->cmd) {
					case eGetVerInfo: {
						BoardVerInfo_t ver_info;

						memcpy(&ver_info.boot_ver, &(pData->buf[0]),
								sizeof(ver_info.software_ver));
						memcpy(&ver_info.software_ver, &(pData->buf[2]),
								sizeof(ver_info.software_ver));
						memcpy(&ver_info.hardware_ver, &(pData->buf[4]),
								sizeof(ver_info.hardware_ver));
						memcpy(&ver_info.aux_rs485_ver, &(pData->buf[6]),
								sizeof(ver_info.aux_rs485_ver));
						memcpy(&ver_info.modbus_rs485_ver, &(pData->buf[7]),
								sizeof(ver_info.modbus_rs485_ver));
						memcpy(&ver_info.aux_can_ver, &(pData->buf[8]),
								sizeof(ver_info.aux_can_ver));
						memcpy(&ver_info.canopen_ver, &(pData->buf[9]),
								sizeof(ver_info.canopen_ver));
						memcpy(&ver_info.UID, &(pData->buf[10]),
								sizeof(ver_info.UID));
					...
					case eDisableCtrl: {
						...
						memcpy(&realtime_info.MecRawAngle, &(pData->buf[0]),
								sizeof(realtime_info.MecRawAngle)); /* 单圈绝对值角度[0]-[1] */
						memcpy(&realtime_info.MecPosition, &(pData->buf[2]),
								sizeof(realtime_info.MecPosition)); /* 多圈绝对值角度[2]-[5] */
						memcpy(&realtime_info.MecVel_Mulit100, &(pData->buf[6]),
								sizeof(realtime_info.MecVel_Mulit100)); /* 机械速度[6]-[9] */
						memcpy(&realtime_info.Iq_Mulit1000, &(pData->buf[10]),
								sizeof(realtime_info.Iq_Mulit1000)); /* Q轴电流[10]-[13] */
						memcpy(&realtime_info.BusVoltage_multi100,
								&(pData->buf[14]),
								sizeof(realtime_info.BusVoltage_multi100)); /* 母线电压[14]-[15] */
						memcpy(&realtime_info.BusCurrent_multi100,
								&(pData->buf[16]),
								sizeof(realtime_info.BusCurrent_multi100)); /* 母线电流[16]-[17] */
					...
					case eWriteAndSaveMotionCtrlPara: {
						...
						memcpy(&pid.pos_kp, &(pData->buf[0]),
								sizeof(pid.pos_kp));
						memcpy(&pid.pos_ki, &(pData->buf[4]),
								sizeof(pid.pos_ki));

						memcpy(&tmp_val, &(pData->buf[8]), sizeof(tmp_val));
						pid.pos_out_limit = tmp_val * 0.01f;

						memcpy(&pid.vel_kp, &(pData->buf[12]),
								sizeof(pid.vel_kp));
						memcpy(&pid.vel_ki, &(pData->buf[16]),
								sizeof(pid.vel_ki));
