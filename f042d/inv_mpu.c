...
#define delay_ms(val) vTaskDelay(val / portTICK_PERIOD_MS);
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    {
        if (debug)
            ESP_LOGI(TAG, "Reading OTP Register Error.\n");
        return 0x07;
    }
    if (debug)
        ESP_LOGI(TAG, "Accel OTP:%d, %d, %d\n", regs[0], regs[1], regs[2]);
    for (i = 0; i < 3; i++)
    {
        if (regs[i] != 0)
        {
    ...
    else
    {
        ...
        if (debug)
        {
            ESP_LOGI(TAG, "ACCEL:CRITERIA B\r\n");
            ESP_LOGI(TAG, "Min MG: %7.4f\r\n", accel_st_al_min / 1.f);
            ESP_LOGI(TAG, "Max MG: %7.4f\r\n", accel_st_al_max / 1.f);
        }
    ...
    if (result == 0)
    {
        ...
        if (debug)
            ESP_LOGI(TAG, "Accel:CRITERIA C: bias less than %7.4f\n", accel_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > accel_offset_max)
            {
                if (debug)
                    ESP_LOGI(TAG, "FAILED: Accel axis:%d = %ld > 500mg\n", i, bias_regular[i]);
                result |= 1 << i; // Error condition
            }
        }
    }
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
    ...
    else
    {
        ...
        if (debug)
        {
            ESP_LOGI(TAG, "GYRO:CRITERIA B\r\n");
            ESP_LOGI(TAG, "Max DPS: %7.4f\r\n", gyro_st_al_max / 1.f);
        }
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                if (debug)
                    ESP_LOGI(TAG, "GYRO FAIL axis:%d greater than 60dps\n", i);
                result |= 1 << i; // Error condition
            }
        }
    }
    ...
    if (result == 0)
    {
        ...
        if (debug)
            ESP_LOGI(TAG, "Gyro:CRITERIA C: bias less than %7.4f\n", gyro_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > gyro_offset_max)
            {
                if (debug)
                    ESP_LOGI(TAG, "FAILED: Gyro axis:%d = %ld > 20dps\n", i, bias_regular[i]);
                result |= 1 << i; // Error condition
            }
        }
    }
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    if (debug)
        ESP_LOGI(TAG, "Starting Bias Loop Reads\n");
    ...
    if (debug)
    {
        ESP_LOGI(TAG, "Accel offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, accel[0] / 65536.f, accel[1] / 65536.f, accel[2] / 65536.f);
        ESP_LOGI(TAG, "Gyro offset data HWST bit=%d: %7.4f %7.4f %7.4f\r\n", hw_test, gyro[0] / 65536.f, gyro[1] / 65536.f, gyro[2] / 65536.f);
    }
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
    ...
    if (debug)
        ESP_LOGI(TAG, "Starting MPU6500 HWST!\r\n");
    ...
    if (debug)
        ESP_LOGI(TAG, "Retrieving Biases\r\n");
    ...
    if (ii == tries)
    {
         ...
         */
        if (debug)
            ESP_LOGI(TAG, "Retrieving Biases Error - possible I2C error\n");
    ...
    if (debug)
        ESP_LOGI(TAG, "Retrieving ST Biases\n");
    ...
    if (ii == tries)
    {
        ...
        if (debug)
            ESP_LOGI(TAG, "Retrieving ST Biases Error - possible I2C error\n");
    ...
    if (debug)
        ESP_LOGI(TAG, "Accel Self Test Results: %d\n", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if (debug)
        ESP_LOGI(TAG, "Gyro Self Test Results: %d\n", gyro_result);
    ...
    if (debug)
        ESP_LOGI(TAG, "Compass Self Test Results: %d\n", compass_result);
    if (!compass_result)
        result |= 0x04;
#else
    result |= 0x04;
...
static int setup_compass(void)
{
    ...
    if (akm_addr > 0x0F)
    {
        ...
        ESP_LOGE(TAG, "Compass not found.\n");
        return -1;
    }
...
uint8_t mpu_dmp_init(void)
{
    ...
    {
        ESP_LOGI(TAG, "mpu_init successfully");
